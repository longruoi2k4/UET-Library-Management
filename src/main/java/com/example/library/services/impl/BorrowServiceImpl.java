...
/**
 * Implementation of the IBorrowService interface that provides borrow-related services in
 * a library system.
 */
public class BorrowServiceImpl implements IBorrowService {
  ...
  /**
   * Constructs a new instance of the BorrowServiceImpl class.
   * Initializes the service with implementations of BorrowRepository, BookRepository,
   * and ReaderRepository.
   */
  ...
  /**
   * Constructs a new instance of the BorrowServiceImpl class.
   * Initializes the service with implementations of BorrowRepository,
   * BookRepository, and ReaderRepository.
   *
   * @param borrowRepository the repository interface for borrow related operations
   * @param bookRepository the repository interface for book related operations
   * @param readerRepository the repository interface for reader related operations
   */
  public BorrowServiceImpl(
  ...
  public ObservableList<Borrow> getAllRequestByReaderId(String readerId) {
    ObservableList<Borrow> borrows = borrowRepository.getAllBookBorrowed();
    return borrows.filtered(
        borrow -> "REQUEST".equals(borrow.getStatus()) && readerId.equals(borrow.getReaderId()));
  }

  @Override
  public ObservableList<Borrow> getAllRequestBorrow() {
    ObservableList<Borrow> borrows = borrowRepository.getAllBookBorrowed();
    return borrows.filtered(borrow -> "REQUEST".equals(borrow.getStatus()));
  }

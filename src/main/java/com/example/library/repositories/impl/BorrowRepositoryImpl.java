...
/**
 * The BorrowRepositoryImpl class implements the IBorrowRepository interface
 * and provides concrete methods for managing borrowing transactions within the library system.
 * This class handles various operations such as retrieving, saving, updating, and deleting
 * borrowing records,
 * as well as processing borrowing requests, returns, and calculating relevant statistics.
 */
  ...
  public Map<String, String> getAllEmailWithMessagesByBorrowIds(List<String> borrowIds) {
    String sql =
        """
                ...
                """
            .formatted(
                borrowIds.stream().map(id -> "'" + id + "'").collect(Collectors.joining(",")));
    ...
    for (Map.Entry<String, Map<String, List<Map<String, String>>>> entry :
        emailBooksStatusMap.entrySet()) {
      ...
      if (booksByStatus.containsKey("REQUEST")) {
        ...
        message.append(
            "<tr style='background-color: #f2f2f2;'><th style='border: 1px solid #ddd; padding: 8px;'>Book ID</th><th style='border: 1px solid #ddd; padding: 8px;'>Book Name</th><th style='border: 1px solid #ddd; padding: 8px;'>Request Date</th></tr>");

        for (Map<String, String> book : approvedBooks) {
          message
              .append("<tr>")
              .append("<td style='border: 1px solid #ddd; padding: 8px;'>")
              .append(book.get("bookId"))
              .append("</td>")
              .append("<td style='border: 1px solid #ddd; padding: 8px;'>")
              .append(book.get("bookName"))
              .append("</td>")
              .append("<td style='border: 1px solid #ddd; padding: 8px;'>")
              .append(book.get("requestDate"))
              .append("</td>")
              .append("</tr>");
        }
